...
export async function POST(req: Request) {
    try {
        ...
        const hashHex = sha512.create().update(`123456/contest.standings?apiKey=${process.env.API_KEY}&contestId=${contestId}&time=${currentUnixTime}#${process.env.API_SECRET}`).hex()
        ...
        for (let i = 0; i < 10; i++) {
            let name: string
            if (data[i].party.teamId) {
                name = data[i].party.teamName
            }
            else {
                name = data[i].party.members[0].handle
            }
